...
while(minuto <= fin_de_la_jornada): 
    ...
    p = generador_persona()
    if(p):
        caja_mas_vacia = min(cajas, key=len)#obtener la caja con menos personas en cola
        caja_mas_vacia.append(p) #agregar la persona a la cola de la caja más vacía
    ...
    for caja in cajas:
        for persona in caja:
            ...
            if(persona["tiempo_esperando_en_cola"] > persona["grado_de_paciencia"]):
                ...
                print("Una persona abandonó la cola: " + str(persona))
    ...
    for caja in cajas:
        if caja:
            persona_actual = caja[0]
            persona_actual["tiempo_esperando_en_caja"] += 1
            if(persona_actual["tiempo_esperando_en_caja"] > persona_actual["tiempo_que_tarda_la_caja"]):
                caja.remove(persona_actual)
                cantidad_personas_atendidas += 1
                print("Una persona terminó de ser atendida: " + str(persona_actual))


promedio_tiempo_cajas = math.ceil((cantidad_personas_atendidas * 3) / cantidad_de_cajas)
